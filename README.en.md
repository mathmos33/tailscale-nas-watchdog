...
- **Xcode Command Line Tools, version 16.2 or newer (Swift 6.x)** — `build.sh` checks both presence *and* version, and stops with a clear message if missing or too old. Version **15.3 has a known Apple packaging bug** (duplicate modulemap: `redefinition of module 'SwiftBridging'`) that breaks any compile; its `swiftc` (Swift 5.10) is too old regardless. If `xcode-select --install` doesn't open a popup (it sometimes doesn't), use `softwareupdate --list` then `sudo softwareupdate --install "Command Line Tools for Xcode-16.2" --agree-to-license`. Gotcha observed in the wild: macOS's automatic updater can silently reinstall 15.3 over it shortly after — if `swiftc --version` drops back to 5.10, turn off automatic updates (System Settings → General → Software Update) before reinstalling.
