...
- À chaque cycle, tant qu'un volume est monté, le watchdog vérifie qu'un fichier `.metadata_never_index` existe à sa racine et le crée sinon, pour désactiver l'indexation Spotlight dessus (pas besoin de `mdutil`/sudo — utile car Spotlight n'a rien à faire sur des partages réseau, et surtout pas sur celui utilisé pour Time Machine). Ça couvre aussi bien les nouveaux montages que ceux déjà montés avant l'ajout de cette fonctionnalité. Le statut réel (`spotlightDisabled`) est exposé dans `state.json` et affiché dans le menu ("… · Spotlight désactivé").
- Ce même traitement anti-Spotlight est aussi appliqué à **tout** partage SMB monté sous `/Volumes`, même s'il n'est pas listé dans `hosts.json` — par exemple un partage "Films" ou "homes" atteint en parcourant le NAS depuis le Finder plutôt que via `hosts.json`. Ces montages "à la volée" ne sont pas gérés (pas de remontage automatique, pas d'affichage dans le menu), seul le flag Spotlight leur est appliqué.
