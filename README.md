...
*English version: [`README.en.md`](README.en.md). Ce fichier-ci (français) fait foi ; garder les deux synchronisés.*
...
- **Xcode Command Line Tools, version 16.2 ou plus récente (Swift 6.x)** — `build.sh` vérifie sa présence *et* sa version, et s'arrête avec un message clair si absent ou trop ancien. La version **15.3 a un bug connu d'Apple** (modulemap dupliqué : `redefinition of module 'SwiftBridging'`) qui empêche toute compilation ; son `swiftc` (Swift 5.10) est de toute façon trop ancien. Si `xcode-select --install` n'ouvre pas de popup (ça arrive), utiliser `softwareupdate --list` puis `sudo softwareupdate --install "Command Line Tools for Xcode-16.2" --agree-to-license`. Piège observé : la mise à jour automatique de macOS peut réinstaller silencieusement la 15.3 par-dessus peu après — si `swiftc --version` retombe sur 5.10, couper les mises à jour auto (Réglages Système → Général → Mise à jour de logiciels) avant de réinstaller.
...
`build.sh` compile avec `-target <arch>-apple-macosx11.0` (`<arch>` = `arm64` ou `x86_64` selon `uname -m`). Le `-target` explicite est nécessaire : sans lui, `swiftc` embarque par défaut la version du SDK installé (28.0 sur cette machine, plus récente que l'OS réellement installé en 27.0), ce qui fait planter le lancement via Finder avec *"Impossible d'utiliser cette version de l'application avec cette version de macOS"* — le lancement direct (`launchctl`) contournait ce contrôle, mais pas Finder. Fixer la version macOS du `-target` à 11.0 élimine le problème avec une large marge, pas besoin d'ajuster à chaque mise à jour de macOS.

Sans signature, Finder refuse carrément de lancer l'app (icône barrée d'un signe interdit), même sans flag de quarantine — Gatekeeper sur cette bêta semble plus strict que d'habitude pour les apps locales non signées. L'app est signée avec un **certificat auto-signé stable** (`TailscaleNAS Local Signing`), généré une seule fois par machine dans le Trousseau de connexion et réutilisé à chaque rebuild (`build.sh` le détecte via `security find-identity` et le recrée seulement s'il manque). Une signature **ad-hoc** (`--sign -`) aurait été plus simple, mais elle change de hash à chaque rebuild, ce qui invalide silencieusement les autorisations TCC déjà accordées à l'app (Accès complet au disque, Volumes réseau) — donc le flag `.metadata_never_index` (voir plus bas) arrêtait de se poser après chaque `install.sh`. Avec le certificat stable, la *designated requirement* reste liée au certificat (`certificate leaf = H"…"`) plutôt qu'au binaire, donc les autorisations survivent aux rebuilds.

Si tu dois régénérer le certificat depuis zéro (perdu, corrompu, etc.) : `security delete-certificate -c "TailscaleNAS Local Signing" ~/Library/Keychains/login.keychain-db`, puis relance `build.sh` — il en recrée un nouveau. Il faudra alors re-accorder les autorisations TCC une dernière fois (voir section suivante).
